...
O uso dessas novas plataformas tem se difundido em grande parte pelo seu custo acessível, mas também pelos constantes incrementos em suas capacidades de processamento, o que as habilita a desempenhar tarefas cada vez mais complexas. Nesse contexto, propõe-se a avaliação de desempenho computacional de um algoritmo, não paramétrico e estatístico descrito em \cite{IEEEhowto:horprasert}, embarcado na plataforma Raspberry PI 2 (RPI2) e CubieBoard 2 (CB2), a fim de que se tenha uma estimativa da capacidade destas plataformas de processar imagens e efetuar a subtração do fundo em tempo real, a 30 quadros por segundo, com a máxima resolução possível.
...
\section{Resultados}
A figura \ref{fig:sim_pc} mostra o resultado da simulação feita em um Notebook com processador Intel Core I5 utilizando como câmera a sua Webcam integrada. Também foram feitos testes no RPI2 utilizando-se o mesmo código desenvido e utilizando um módulo de câmera próprio para a plataforma como fonte de imagens. A tabela \ref{tab:table_pc_rpi} mostra os primeiros resultados obtidos quando da execução do algoritmo implementado em Python tanto em um ambiente com muitos recursos de processamento quanto em um ambiente com severas restrições como o RPI2.

\begin{figure}[!t]
\centering
\includegraphics[width=2.5in]{Screen_PC_01}
\caption{Resultado de simulação em Computador Intel Core I5}
\label{fig:sim_pc}
\end{figure}
...
\renewcommand{\arraystretch}{1.3}
\caption{Resultados de Testes em Python}
...
\multirow{2}{*}{Notebook I5}		& 320x240 & 25 - 35	\\ \cline{2-3}
								& 640x480 & 13 - 18	\\ \hline
\multirow{2}{*}{Raspberry Pi 2}	& 320x240 & 5 - 7	\\ \cline{2-3}
								& 640x480 & 1 - 3	\\ \hline
